...
export default function TaskCard({ task, done, note, phaseColor, catColor, lvlColor, onToggle, onNote }) {
  ...
  return (
    ...
    >
      <div onClick={handleToggle} className="flex items-center gap-3 px-4 py-3">
        {/* Checkbox */}
        <div
          ref={checkRef}
          className={`w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center text-sm font-extrabold transition-all duration-300
            ${done
              ? 'bg-[var(--ok)] border-2 border-[var(--ok)] text-white'
              : 'bg-[var(--bg2)] border-2 border-[var(--bd)] text-transparent'
            }`}
        >
          ✓
        </div>
        ...
        {/* Day badge */}
        <div
          className="text-white font-extrabold text-[10px] rounded-md px-2 py-1 min-w-[42px] text-center font-mono flex-shrink-0"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1 mt-1 flex-wrap">
            <span className="text-[9px] font-bold text-[var(--tx3)] bg-[var(--bg3)] px-1.5 py-0.5 rounded">
              {task.phase}
            </span>
            ...
            <span className="text-[9px] font-bold text-[var(--ac)] bg-[var(--ac)]/10 px-1.5 py-0.5 rounded">
              +{XP_PER_TASK[task.lvl]}XP
            </span>
        ...
        {/* Note button */}
        <button
          onClick={e => { e.stopPropagation(); setShowNote(!showNote); setNoteText(note || '') }}
          className="w-7 h-7 rounded-md bg-[var(--bg3)] text-[var(--tx3)] text-xs flex items-center justify-center flex-shrink-0 hover:bg-[var(--bd)] transition-colors"
        >
          📝
        </button>
      </div>
